...
build_gpgme() {
  ...
  download "https://gnupg.org/ftp/gcrypt/gpgme/gpgme-2.2.0.tar.bz2" "$tarball" || exit 1
  tar xf "$tarball" || exit 1
  rm -f "$tarball"
  cd gpgme-* || exit 1
...
build_libpsl() {
  ...
  download "https://github.com/rockdaboot/libpsl/releases/download/0.23.3/libpsl-0.23.3.tar.gz" "$tarball" || exit 1
  tar xzf "$tarball" || exit 1
  rm -f "$tarball"
  cd libpsl-* || exit 1
