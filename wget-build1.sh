...
if [[ "$ssl_type" == "gnutls" ]]; then
  ...
  wget -q -O wget-gnutls-deps.tar.zst https://github.com/rzhy1/wget-windows/releases/download/wget-1.25.0/wget-gnutls-deps.tar.zst
  tar -I zstd -xf wget-gnutls-deps.tar.zst -C "$PWD"
  build_wget_gnutls
else
  echo ">>> 下载并解压 OpenSSL 依赖..."
  wget -q -O wget-openssl-deps.tar.zst https://github.com/rzhy1/wget-windows/releases/download/wget-1.25.0/wget-openssl-deps.tar.zst
