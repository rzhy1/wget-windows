...
# 创建 wget 补丁文件来修复 openat-die.c 问题
create_wget_patch() {
    cat > wget_mingw_fix.patch << 'EOF'
--- a/lib/openat-die.c
+++ b/lib/openat-die.c
@@ -19,6 +19,10 @@
...
create_wget_patch
echo "应用 MinGW 兼容性补丁..."
patch -p1 < wget_mingw_fix.patch
(($? != 0)) && { printf '%s\n' "[wget gnutls] patch failed"; exit 1; }
...
echo "应用 MinGW 兼容性补丁..."
patch -p1 < wget_mingw_fix.patch
(($? != 0)) && { printf '%s\n' "[wget openssl] patch failed"; exit 1; }
