...
if [ ! -f "$INSTALL_PATH"/lib/libexpat.a ]; then
  wget -O- https://github.com/libexpat/libexpat/releases/download/R_2_6_3/expat-2.6.3.tar.gz | tar xz
  cd expat-* || exit
  ./configure \
  --host=$WGET_MINGW_HOST \
  --disable-shared \
...
if [[ "$ssl_type" == "gnutls" ]] && [ ! -f "$INSTALL_PATH"/lib/libgnutls.a ]; then
  wget -O- https://www.gnupg.org/ftp/gcrypt/gnutls/v3.8/gnutls-3.8.7.1.tar.xz | tar x --xz
  cd gnutls-* || exit
...
if [[ "$ssl_type" == "gnutls" ]]; then
  ...
  # patch src/openssl.c < windows-openssl.diff
   CFLAGS="-I$INSTALL_PATH/include -DCARES_STATICLIB=1 -DPCRE2_STATIC=1 -DNDEBUG -O3 -pipe -march=$WGET_ARCH -mtune=generic" \
   LDFLAGS="-L$INSTALL_PATH/lib -static -static-libgcc" \
