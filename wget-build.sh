...
# --- 核心编译参数定义 ---
export CFLAGS="-march=x86-64-v3 -O2 -pipe -ffunction-sections -fdata-sections -fuse-linker-plugin -fvisibility=hidden -fno-stack-protector -fomit-frame-pointer -DNDEBUG -flto=$NPROC"
...
build_libpsl() {
  ...
  if [ ! -f "$INSTALL_PATH"/lib/libpsl.a ]; then
    cd "$INSTALL_PATH" || exit 1
    rm -rf libpsl-*
    wget -q -O- https://github.com/rockdaboot/libpsl/releases/download/0.23.3/libpsl-0.23.3.tar.gz | tar xz
    cd libpsl-* || exit 1
...
build_pcre2() {
  ...
  if [ ! -f "$INSTALL_PATH"/lib/libpcre2-8.a ]; then
    cd "$INSTALL_PATH" || exit 1
    rm -rf pcre2-*
    wget -q -O- https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.48/pcre2-10.48.tar.gz | tar xz
    cd pcre2-* || exit 1
    LDFLAGS="$LDFLAGS_DEPS" ./configure --host=$WGET_MINGW_HOST --disable-shared --prefix="$INSTALL_PATH" --enable-static
    make -j$NPROC && make install
  fi
...
build_openssl() {
  ...
  if [ ! -f "$INSTALL_PATH"/lib/libssl.a ]; then
    ...
    CFLAGS="-march=x86-64-v3 -Os -ffunction-sections -fdata-sections -pipe -g0 -fvisibility=hidden $LTO_FLAGS" \
